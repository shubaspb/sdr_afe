function h_pulse = compensator_generate(Fs, Fc, M, N, R, L)
%L = 25;                                 %% Filter order; must be even
%%%%%% CIC filter parameters %%%%%%
% R = 4;                                   %% Decimation factor
% M = 8;                                   %% Differential delay
% N = 4;                                   %% Number of stages
% B = 18;                                  %% Coeffi. Bit-width
% Fs = 100e6;                           %% (High) Sampling freq in Hz before decimation
% Fc = 5e6;                             %% Pass band edge in Hz
%%%%%%% fir2.m parameters %%%%%%
...
%h_pulse = round(h*power(2,B-1)-1); %% Fixed point coefficients
